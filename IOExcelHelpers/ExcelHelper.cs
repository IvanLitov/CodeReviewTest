...
namespace IOExcelHelpers
{
    public static class ExcelHelper
    {
        ...
        private static Borders _borders;
        private static Fills _fills;
        private static CellStyleFormats _cellStyleFormats;
        private static CellFormats _cellFormats;
        private static NumberingFormats _numberingFormats;
        private static CellStyles _cellStyles;
        ...
        private const string FileRejectedMsg = "File rejected: ";
        ...
        public static void ReadTemplateIntoTable(DataSet dataSet, string fileName, List<string> supportedSheetNames, bool flag)
        {
            using (var spredsheetDocument = SpreadsheetDocument.Open(fileName, false))
            {
                ...
                _colors = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.Colors;
                _fonts = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.Fonts;
                _borders = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.Borders;
                _fills = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.Fills;
                _cellStyleFormats = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.CellStyleFormats;
                _cellStyles       = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.CellStyles;
                _numberingFormats = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.NumberingFormats;
                _cellFormats = workBookRoot.WorkbookPart.WorkbookStylesPart.Stylesheet.CellFormats;
                ...
                foreach (var sh in sheets)
                {
                    ...
                    if (!supportedSheetNames.Contains(sheetName.ToUpper()))
                        continue;
        ...
        public static void ReadDictionaryIntoTable(DataSet dataSet, string fileName, List<string> supportedSheetNames)
        {
            using (var spredsheetDocument = SpreadsheetDocument.Open(fileName, false))
            {
                ...
                foreach (var sh in sheets)
                {
                    ...
                    dataTable.TableName = sheetName;
                    dataTable.Columns.Add(DictionaryColumnName);
                    dataTable.Columns.Add(DictionaryDataType);
                    ...
                    if (worksheetPart != null)
                    {
                        ...
                        {
                            if (rowCount == 1)                  // skipping the 1st row with the column names
                            {
                                rowCount++;
                                continue;
                            }
                            ...
                            {
                                if (columnCount > 2)            // skipping the rest columns after the 2nd
                                    continue;

                                var cell = (Cell)openXmlElement;
                                var numberFormatId = GetNumberFormatIdFromCellStyleIndex(cellFormats, cell);
                                var cellValueStr = GetCellValue(spredsheetDocument, cell, numberFormatId);

                                if (columnCount == 1)
                                {
                                    tableRow[columnCount - 1] = cellValueStr.Replace(" ", "");
                                }
                                else
                                {
                                    tableRow[columnCount - 1] = cellValueStr;
                                }
                ...
                foreach (var sh in sheets)
                {
                    ...
                    if (worksheetPart != null)
                    {
                        ...
                        foreach (var row in rows)                                   // DocumentFormat.OpenXml.Spreadsheet.Row
                        {
                            ...
                            if (rowIndex == 1)      // Note - Dont't use the row.RowIndex.Value !!!
                            {
                                if (row.Descendants() == null)
                                {
                                    continue;
                                }
                                ...
                                foreach (var cell in row.Descendants<Cell>())
                                {
                                    ...
                                    if (cell.CellReference != null)
                                    {
                                        ...
                                        if (!dt.Columns.Contains(NewColumnName(cellRef, columnName, true)))
                                        {
                                            isExcelFileStructureValid = false;
                                            errorsOnInvalidCoulumns.Add(sheetName + Constants.Slash + NewColumnName(cellRef, columnName, true));
                                        }
                                    }
                                    else
                                    {
                                        var columnName = cell.CellValue.Text;
                                        columnNames.Add(columnName);

                                        if (!dt.Columns.Contains(columnName))
                                        {
                                            isExcelFileStructureValid = false;
                                            errorsOnInvalidCoulumns.Add(sheetName + Constants.Slash + columnName);
                                        }
                                    }
                                }
                            }
                            else if (isExcelFileStructureValid)
                            {
                                if (!row.HasChildren)
                                {
                                    break;
                    ...
                    try
                    {
                        ...
                        foreach (var sh in sheets)
                        {
                            ...
                            if (worksheetPart != null)
                            {
                                ...
                                if (dt == null)
                                {
                                    _errorsOnInvalidCoulumns.Add(string.Format(FileRejectedMsg + "The sheet {0} is not supported in this template", sheetName));
                                    continue;
                                }
                                                ...
                                                if (tmpCellValue == "ref" || tmpCellValue == "map4" || tmpCellValue == "map5")
                                                {
                                                    ...
                                                    if (cell.CellReference == "A2" && (cellFastLenderLtd.CellValue == null))
                                                    {
                                                        if (sheetName == Constants.Accounts)
                                                            _errorsOnInvalidCoulumns.Add(FileRejectedMsg + string.Format("The row {0} does not have any value in the 'Primary Loan ID' column", cellFastLenderLtd.CellReference));
        ...
        private static int MapFastLenderLtdPropertyTypes(string appId, string propType)
        {
            ...
            {
                // validation error
                _errorsOnInvalidCoulumns.Add(string.Format(FileRejectedMsg + "The '{0}' value in column '{1}' is invalid for Application = {2}", propType, fieldName, appId));
                result = 63;
            }
        ...
        private static string MapEnumStringValue(string appId, string enumValue, string fieldName, string result)
        {
            ...
            {
                // validation error
                _errorsOnInvalidCoulumns.Add(string.Format(FileRejectedMsg + "The '{0}' value in column '{1}' is invalid for Application = {2}", enumValue, fieldName, appId));
                result = enumValue;
            }
        ...
        private static uint GetNumberFormatIdFromCellStyleIndex(CellFormats cellFormats, Cell cell)
        {
            ...
            var cellFormat = cellFormats.ElementAt((int)styleIndex);

            if (cellFormat == null)
                return numberFormatIdValue;

            numberFormatIdValue = ((CellFormat)cellFormat).NumberFormatId.Value;
        ...
        private static string GetCellValue(SpreadsheetDocument document, Cell cell, uint numberFormatId, string formatCode = "")
        {
            if (cell.CellValue == null)
                return "";
        ...
        private static void CreateStyleSheet(Workbook workbook)
        {
            ...
            if (_numberingFormats != null)
            {
                ...
                {
                    Count = _numberingFormats.Count
                };
                foreach (var openXmlElement in _numberingFormats)
                {
            ...
            if (_colors != null)
            {
                ...
                foreach (var color in _colors.MruColors)
                {
            ...
            if (_fonts != null)
            {
                ...
                foreach (var openXmlElement in _fonts)
                {
            ...
            if (_fills != null)
            {
                ...
                foreach (var openXmlElement in _fills)
                {
            ...
            if (_borders != null)
            {
                ...
                foreach (var openXmlElement in _borders)
                {
            ...
            if (_cellStyleFormats != null)
            {
                var cellStyleFormats = new CellStyleFormats ();        // <Appending CellFormats>
                foreach (var openXmlElement in _cellStyleFormats)
                {
            ...
            if (_cellFormats != null)
            {
                ...
                foreach (var openXmlElement in _cellFormats)
                {
            ...
            if (_cellStyles != null)
            {
                ...
                foreach (var openXmlElement in _cellStyles)
                {
